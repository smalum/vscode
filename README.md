...
Маркетплейс не блокирует использование: после **A3** раздаём `.vsix` (`vsce package` → GitHub Releases), автообновление для Cursor/VSCodium — Open VSX. `publisher` и `name` фиксируем на **A3.5**: переименование после публикации даст пользователям второй плагин рядом со старым.
